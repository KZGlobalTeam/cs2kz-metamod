...
{
public:
	virtual bool OnTimerStart(KZPlayer *player, const char *courseName) { return true; }
	virtual void OnTimerStartPost(KZPlayer *player, const char *courseName) {}
	virtual bool OnTimerEnd(KZPlayer *player, const char *courseName, f32 time, u32 teleportsUsed) { return true; }
	virtual bool OnTimerEndMessage(KZPlayer *player, const char *courseName, f32 time, u32 teleportsUsed) { return true; }
	...
	virtual bool OnPause(KZPlayer *player) { return true; }
	virtual void OnPausePost(KZPlayer *player) {}
	virtual bool OnResume(KZPlayer *player) { return true; }
...
public:
	...
	bool GetTimerRunning() { return timerRunning; }
	bool GetValidTimer() { return validTime; }
	f64 GetTime() { return currentTime; }
	static_global void FormatTime(f64 time, char *output, u32 length, bool precise = true);

	void SetTime(f64 time) { currentTime = time; timerRunning = time > 0.0f; }
	void GetCourse(char *buffer, u32 size) { V_snprintf(buffer, size, "%s", currentCourse); }
	void SetCourse(const char *name) { V_strncpy(currentCourse, name, KZ_MAX_COURSE_NAME_LENGTH); }
	enum TimeType_t
	{
		TimeType_Standard,
		TimeType_Pro
	};
	TimeType_t GetCurrentTimeType() { return this->player->checkpointService->GetTeleportCount() > 0 ? TimeType_Standard : TimeType_Pro; }
...
private:
	...
	static_global bool IsValidMoveType(MoveType_t moveType) { return moveType == MOVETYPE_WALK || moveType == MOVETYPE_LADDER	|| moveType == MOVETYPE_NONE || moveType == MOVETYPE_OBSERVER; }
	bool JustTeleported();
	bool JustLanded() { return g_pKZUtils->GetGlobals()->curtime - this->player->landingTime < KZ_TIMER_MIN_GROUND_TIME; }
	bool JustStartedTimer() { return timerRunning && this->GetTime() < EPSILON; }
	bool JustEndedTimer();
...
public:
	...
	void SetPausedOnLadder(bool ladder) { pausedOnLadder = ladder; }
	void Pause();
