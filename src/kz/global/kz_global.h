...
private:
	...
	{
		// Replays are uploaded with `POST {apiUrl}/records/{record_id}/replay`, authorized by the
		// single-use key from the record's `new-record` ack.
		...
		static constexpr u32 maxUploadAttempts = 3;
		static constexpr std::chrono::seconds uploadRetryDelay = std::chrono::seconds(2);

		std::mutex mutex;
		std::vector<PendingUpload> pendingUploads;

		void QueueUpload(PendingUpload &&upload);
		void ProcessUploads();
		static void RetryUpload(std::shared_ptr<PendingUpload> upload, const char *reason);
...
public:
	static void QueueReplayUpload(const UUID_t &recordID, const std::string &uploadKey, std::vector<char> &&replayData)
	{
		replayManager.QueueUpload({recordID, uploadKey, std::move(replayData)});
	}

	/**
	 * Reads `apiUrl` from the server config and validates it. The result always ends with a `/`.
	 */
	static bool GetApiHttpUrl(std::string &url);
