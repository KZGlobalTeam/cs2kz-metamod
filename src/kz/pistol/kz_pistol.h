#pragma once
#include "../kz.h"

struct PistolInfo_t
{
	i16 itemDef;
	i8 team;
	const char *name;
	const char *className;
	std::vector<const char *> aliases;
};

class KZPistolService : public KZBaseService
{
public:
	static inline const std::vector<PistolInfo_t> pistols = {
		{-1, CS_TEAM_NONE, "Disabled", "", {"knife", "disable", "disabled", "none", "0"}},
		{1, CS_TEAM_NONE, "Desert Eagle", "weapon_deagle", {"deagle", "de", "deag"}},
		{2, CS_TEAM_NONE, "Dual Berettas", "weapon_elite", {"elite", "dualies", "dual berettas"}},
		{3, CS_TEAM_CT, "Five-SeveN", "weapon_fiveseven", {"fiveseven", "57"}},
		{4, CS_TEAM_T, "Glock-18", "weapon_glock", {"glock", "glock18"}},
		{30, CS_TEAM_T, "Tec-9", "weapon_tec9", {"tec9"}},
		{32, CS_TEAM_CT, "P2000", "weapon_hkp2000", {"p2000"}},
		{36, CS_TEAM_NONE, "P250", "weapon_p250", {"p250"}},
		{61, CS_TEAM_CT, "USP-S", "weapon_usp_silencer", {"usps", "usp-s", "usp"}},
		{63, CS_TEAM_NONE, "CZ75-Auto", "weapon_cz75a", {"cz75-auto", "cs75a", "cz"}},
		{64, CS_TEAM_NONE, "R8 Revolver", "weapon_revolver", {"r8 revolver", "r8revolver", "revolver", "r8"}}};

	static int GetPistolIndexByName(const char *name)
	{
		if (!name)
		{
			return -1;
		}
		for (i16 i = 0; i < pistols.size(); i++)
		{
			if (KZ_STREQI(pistols[i].name, name) || KZ_STREQI(pistols[i].className, name))
			{
				return i;
			}
			for (const char *alias : pistols[i].aliases)
			{
				if (KZ_STREQI(alias, name))
				{
					return i;
				}
			}
		}
		return -1;
	}

	using KZBaseService::KZBaseService;
	static void Init();

	virtual void Reset() override
	{
		this->preferredPistol = 8; // Default to USP-S
	}

	void OnPlayerSpawn()
	{
		this->UpdatePistol();
	}

	void UpdatePistol();
	bool HasCorrectPistolEquipped();
	// Return true if the player has a weapon that isn't a knife.
	bool NeedWeaponStripping();
	i16 preferredPistol = 8; // Default to USP-S
};
