...
#define SCHEMA_FIELD_OFFSET(type, varName, extra_offset)															\
	...
	{																												\
		...
		{																											\
			...
			schema::NetworkStateChanged((uintptr_t)(this) + m_chain, m_key.offset + extra_offset, 0xFFFFFFFF);	\
		}																											\
		...
		{																											\
			/* WIP: Works fine for most props, but inlined classes in the middle of a class will
				need to have their this pointer corrected by the offset .*/											\
					Msg("Attempting to call SetStateChanged on on %s::%s\n", ThisClass, #varName);						\
					if (!IsStruct)																							\
						SetStateChanged((CBaseEntity2 *)this, m_key.offset + extra_offset);									\
